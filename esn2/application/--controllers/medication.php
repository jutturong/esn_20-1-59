...
*/
       ...
       public function  fetchMedi()
       {
          ...
          $MonitoringDate_medi=$d."/".$m."/".$y;
           
           
           
           
           $tb=$this->tb_main;
           $query=$this->db->get_where($tb,array('HN'=>$HN,'MonitoringDate'=>$MonitoringDate_medi));
           //$query=$this->db->get_where($tb,array('HN'=>$HN));
           $row=$query->row();
              if( $row )
              {
                  echo json_encode(array(
                      'HN_medi'=>$HN,
                      'MonitoringDate_medi'=>$MonitoringDate_medi,
                      'DRPselection4'=>$row->DRPselection4,
                      'MedicationErrorDrug4'=>$row->MedicationErrorDrug4,
                      'MedicationErrorDetail'=>$row->MedicationErrorDetail,
                      'Action4'=>$row->Action4,
                      'Response4'=>$row->Response4,
                      'ResponseDetail4'=>$row->ResponseDetail4,
                      'followup_medi'=>$row->followup,
                      'week_medi'=>$row->week,
