	 ...
	 switch($appen)
	 {
		...
		{ 
				...
				//	$pdf->MultiCell( 0  , 0 , iconv( 'UTF-8','cp874' , '(Appendix 1) แบบบันทึกข้อมูลพื้นฐานของผู้ป่วยเมื่อเริ่มการรักษา (first visit)' )  );
                   ...
                   $pdf->setXY( $x_absolute, $y_absolute + ($r*11) );//วัน-เดือน-ปี เกิดต้องทำการแก้ไข เป็น   วัน-เดือน-ปี เกิด
				   $ex=explode('-',$birthdate);
				   $con_y=$ex[0]+543;
				   $con_br=$ex[2].'/'.$ex[1].'/'.$con_y;
				   $pdf->MultiCell( 0  , 0 , iconv( 'UTF-8','cp874' , 'วัน เดือน ปี เกิด : '.$con_br.'' ) );
        ...
        case  3:
		{
		  ...
		  $r=7;
			 $tb="tb_appendix1";
				    ...
				    while($row=mysql_fetch_object($q_qpp3))
					{
...
case  6:
		{
		  ...
		  $r=7;
			 $tb="tb_appendix1";
					...
					$q_qpp6=mysql_query($ap6);
				    while($row=mysql_fetch_object($q_qpp6))
					{
